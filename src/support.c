...
gboolean parse_dvd(gchar * filename)
{
    ...
    if (g_ascii_strncasecmp(filename, "dvd://", strlen("dvd://")) == 0) {       // || g_ascii_strncasecmp(filename,"dvdnav://",strlen("dvdnav://")) == 0) {
        ...
        if (error_msg != NULL) {
            ...
            gtk_window_set_title(GTK_WINDOW(dialog), _("WGmplayer Error"));
...
void retrieve_metadata(gpointer data, gpointer user_data)
{
    ...
    Wg_mutex_unlock(&retrieve_mutex);
...
gboolean save_playlist_pls(gchar * uri)
{
    ...
    if (data != NULL) {
        ...
        gtk_window_set_title(GTK_WINDOW(dialog), "WGmplayer Error");
        gtk_dialog_run(GTK_DIALOG(dialog));
        gtk_widget_destroy(dialog);
...
gchar *get_localfile_from_uri(gchar * uri)
{
    ...
    if (localfile == NULL) {
        ...
        tmp = g_strdup_printf("%s/wgmplayer", g_get_user_cache_dir());
        ...
        if (localfile == NULL) {
            ...
            if (base != NULL) {
                ...
                Wg_mutex_lock( &(idledata->caching) );
                ...
                Wg_mutex_unlock( &(idledata->caching) );
